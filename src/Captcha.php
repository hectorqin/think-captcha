...
class Captcha
{
     ...
     */
    protected function configure($configName = null)
    {
        $isTP5 = version_compare(\think\App::VERSION, '6.0.0', '<');
        if (!$configName) {
            $config = $isTP5 ? Config::pull('captcha') : Config::get('captcha');
        } else {
            $config = Config::get('captcha.' . $configName, []);
        }
        $config = array_merge(is_array($config) ? $config : [], $this->config);
    ...
    public function check($code, $id = '')
    {
        ...
        if (password_verify($code, $secode['verify_code'])) {
     ...
     * @param string $configName 验证码配置
     * @return \think\Response
     */
    public function entry($id = '', $configName = null)
    {
        $this->configure($configName);
        // 图片宽(px)
        ...
        // 保存验证码
        Session::set(md5($this->seKey) . $id, [
            'verify_code' => $generator['key'],
            'verify_time' => time(),
        ]);
